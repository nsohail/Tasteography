...
function callback(results, status){
    ...
    for(var i = 0; i < results.length; i++){
        ...
        if((!photoForPlace) || (photoForPlace === undefined)){ //if there are no pics
            ...
            $('.results').append("<li data-id = '"+results[i].place_id+"'><div class='food-thumbnail'><img style='height:190px;min-width: 230px;' src='"+photoForPlace+"'></div><div class='resultName'>"+placeName+"</div><div class='icongroup'><div class='icon'><img src='images/location.png' style='width:10px; margin-right: 10px;'></div><div class='location'>City Not Available</div></div></li>");
        }
        ...
        google.maps.event.addListener(marker, 'click', function() {
            infowindow.setContent(this.name + '<img src="'+this.icon+'">');
